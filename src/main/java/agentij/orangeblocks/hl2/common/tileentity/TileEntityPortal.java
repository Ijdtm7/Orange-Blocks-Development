package agentij.orangeblocks.hl2.common.tileentity;
...
public class TileEntityPortal extends TileEntity implements ITickable
{
    ...
    public void update()
    {
        ...
        if(!world.isRemote)
        {
            if(!Main.eventHandlerServer.getSaveData(world).portalInfo.containsKey(world.provider.getDimension()))
            {
                return;
            }
            PortalInfo info = Main.eventHandlerServer.getSaveData(world).portalInfo.get(world.provider.getDimension()).get(orange ? "blue" : "orange");
            if(info == null)
            {
                return;
            }
        ...
        {
            if(orange && !Main.eventHandlerClient.status.blue || !orange && !Main.eventHandlerClient.status.orange)
            {
                return;
            }
        }
    ...
    public void teleport(Entity ent, TileEntityPortal pair)
    {
        ...
        Main.channel.sendToAllAround(new PacketEntityLocation(ent), new NetworkRegistry.TargetPoint(ent.getEntityWorld().provider.getDimension(), pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, 256));
    }
